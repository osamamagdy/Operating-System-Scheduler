...
void HPF_SC(int msgq_id, int sem1, int sem2, FILE* sc_logs, FILE* sc_perf, int capacity)
{
    // ready queue and state variables
    ...
    while (1)
    {
        ...
        if (!CPU_working && ready_queue->count != 0)
        {
            ...
            current_priority = current_p->priority;
            CPU_working = true;
        ...
        while (rec_val != -1)
        {
            ...
            printf("Scheduler : At time %d New process arrived with id = %d\n", x, message.p.id);
            pid = fork();
            if (pid == 0)
            {
                printf("Forked\n");
            ...
            else
            {
                ...
                *temp_p = message.p;
                ...
                if (current_priority != INT_MAX && current_priority > temp_p->priority)
                {
